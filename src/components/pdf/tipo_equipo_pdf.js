...
export function tipo_equipo_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    doc.setProperties({
      title: "Registro de Tipo de Equipos",
    });
    cabecera();
    data.map((datos, index) => {
      ...
      doc.text(datos.id_tequipo, 20, lineas);
      doc.text(datos.nombre_tequipo, 34, lineas);
      doc.text(datos.nombre_categoria, 100, lineas);
  ...
  function cabecera() {
    ...
    doc.text("Registro de Tipo de Equipos", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Nombre Tipo de Equipo", 50, 25, { align: "center" });
    doc.line(95, 19.8, 95, 27.2);
    doc.text("Categoría", 109, 25, { align: "center" });
