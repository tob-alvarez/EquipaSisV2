...
import {
  ...
  ayuda_servicios,
  trae_permisos
} from "./funciones_servicio";
import { servicio_pdf, servicio_xls } from "../pdf/servicio_pdf";
...
const Servicio = () => {
  ...
  const [datos_servicios, setDatosservicios] = useState([]);
  ...
  useEffect(() => {
      trae_servicios().then((result) => setDatosservicios(result));
      switch (idioma) {
        case "es":
          ayuda_servicios().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_servicios().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_servicios().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_servicios().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarEnGrilla(expresiones) {
    return datos_servicios?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_servicios.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    servicio_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    servicio_xls(searchTerm);
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-2 container">
        ...
        <h1 className="m-0">{t("servicio.titulo")}</h1>
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("servicio.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("servicio.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
