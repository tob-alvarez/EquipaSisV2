...
export function estado_solicitud_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    doc.setProperties({
      title: "Registro de Estados de Solicitudes",
    });
    cabecera();
  ...
  function cabecera() {
    ...
    doc.text("Registro de Estados de Solicitudes", 15, 12);
    ...
    doc.text("Color", 109, 25, { align: "center" });
...
export function estado_solicitud_xls(filtro) {
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "estado_solicitudes");
