...
import { cambia_tipo_equipos } from "./funciones_tipo_equipo";
...
const ModalEditar = ({dato}) => {
    ...
    useEffect(() => {
      if (isModalEditOpen && dato) {
        ...
        setNombre_tequipo(dato.nombre_tequipo);
        setId_categoria(dato.id_categoria);
        setId_prioridad(dato.id_prioridad);
        setId_tcontrol(dato.id_tcontrol);
      }
    ...
    const acepta_accion = () => {
      ...
      if (nombre_tequipo == "" || id_categoria == "" || id_prioridad == "" || id_tcontrol == "") {
        toast.info(`${t("tipo_equipo.datoObligatorio")}`);
        return;
      }
      
      cambia_tipo_equipos(datos_cambios).then((respuesta_accion) => {
