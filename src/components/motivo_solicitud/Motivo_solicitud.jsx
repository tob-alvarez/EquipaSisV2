...
import './table.css'
import {
  trae_motivo_solicitudes,
  ayuda_motivo_solicitudes,
  trae_permisos
} from "./funciones_motivo_solicitud";
import { motivo_solicitud_pdf, motivo_solicitud_xls } from "../pdf/motivo_solicitud_pdf";
...
const Motivo_solicitud = () => {
  const [t] = useTranslation("global")
  const [datos_motivo_solicitudes, setDatosmotivo_solicitudes] = useState([]);
  ...
  useEffect(() => {
      trae_motivo_solicitudes().then((result) => setDatosmotivo_solicitudes(result));
      switch (idioma) {
        case "es":
          ayuda_motivo_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_motivo_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_motivo_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_motivo_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_motivo_solicitud.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_motivo_solicitud.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_motivo_solicitudes?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_motivo_solicitudes.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    motivo_solicitud_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    motivo_solicitud_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("motivo_solicitud.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("motivo_solicitud.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
...
export default Motivo_solicitud