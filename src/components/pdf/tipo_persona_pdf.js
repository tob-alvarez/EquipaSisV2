...
export function tipo_persona_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    let reporte;
    if (idioma === 'es') {
      titulo = "Registro de Tipo de Personas";
      nombreAcciones = "Nombre de Tipo de Persona";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Type of People";
      nombreAcciones = "Person Type Name";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas do Tipo de Pessoas";
      nombreAcciones = "Nome do Tipo de Pessoa";
      habilitado = "Habilitado";
      page = "Página";
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_tipo_persona" }); // Send the data to the server in JSON format.
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_tpersona.toLowerCase().indexOf(filtro) > -1 || 
    item.id_tpersona.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_tpersona.length > 120) {
      ...
      doc.text(datos.id_tpersona, 20, lineas);
      doc.text(datos.nombre_tpersona, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_tpersona.length > 100) {
...
export function tipo_persona_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "tipo_personas");

      /* Export to file (start a download) */
      XLSX.writeFile(wb, fecha + "_Tipo_persona.xlsx");
    }
  };
  resultado()
}
