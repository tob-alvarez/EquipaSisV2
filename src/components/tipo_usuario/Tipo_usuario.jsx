...
import {
  ...
  ayuda_tipo_usuarios,
  trae_permisos
} from "./funciones_tipo_usuario";
import { tipo_usuario_pdf, tipo_usuario_xls } from "../pdf/tipo_usuario_pdf";
...
const Tipo_usuario = () => {
  ...
  const [datos_tipo_usuario, setDatostipo_usuario] = useState([]);
  ...
  useEffect(() => {
      trae_tipo_usuarios().then((result) => setDatostipo_usuario(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_usuarios().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_usuarios().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_usuarios().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_usuarios().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tusuario.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_tusuario.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.observacion.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_tipo_usuario?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_usuario.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_usuario_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_usuario_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_usuario.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_usuario.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
