...
import './table.css'
import {
  trae_tipo_comprobantes,
  ayuda_tipo_comprobantes,
  trae_permisos
} from "./funciones_tipo_comprobante";
import { tipo_comprobante_pdf, tipo_comprobante_xls } from "../pdf/tipo_comprobante_pdf";
...
const Tipo_comprobante = () => {
  ...
  const [datos_tipo_comprobante, setDatostipo_comprobante] = useState([]);
  ...
  useEffect(() => {
      trae_tipo_comprobantes().then((result) => setDatostipo_comprobante(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_comprobantes().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_comprobantes().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_comprobantes().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_comprobantes().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tcomprobante.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_tcomprobante.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_tipo_comprobante?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_comprobante.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_comprobante_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_comprobante_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_comprobante.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_comprobante.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
