...
import {
  ...
  ayuda_tipo_eventos,
  trae_permisos
} from "./funciones_tipo_evento";
import { tipo_evento_pdf, tipo_evento_xls } from "../pdf/tipo_evento_pdf";
...
const Tipo_evento = () => {
  ...
  const [datos_tipo_evento, setDatostipo_evento] = useState([]);
  ...
  useEffect(() => {
    ...
    consultaPerfil(token)
      trae_tipo_eventos().then((result) => setDatostipo_evento(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_eventos().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_eventos().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_eventos().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_eventos().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tevento.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_tevento.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_tipo_evento?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_evento.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_evento_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_evento_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("tipo_evento.nombre-tevento")}</TableCell>
                <TableCell align="center">{t("tipo_evento.estado")}</TableCell>
                <TableCell align="center">{t("tipo_evento.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_tevento}</TableCell>
                  <TableCell>{dato.nombre_tevento.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_evento.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_evento.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
