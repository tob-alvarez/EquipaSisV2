...
export function categoria_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_categoria.length > 120) {
      ...
      doc.text(datos.id_categoria, 20, lineas);
      doc.text(datos.nombre_categoria, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_categoria.length > 100) {
  ...
  //  doc.addImage(logo, "JPEG", 170, 1, 20, 15); // Agregar la imagen al PDF (X, Y, Width, Height)
    ...
    doc.text("Registro de Categorías", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Nombre de Categoría", 80, 25, { align: "center" });
...
export function categoria_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.writeFile(wb, fecha + "_Categoria.xlsx");
    }
  };
  resultado()
}
