...
export function tipo_comprobante_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    let reporte;
    if (idioma === 'es') {
      titulo = "Registro de Tipo de Comprobante";
      nombreAcciones = "Nombre de Tipo de Comprobante";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Document Type";
      nombreAcciones = "Document Type";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas do Tipo de Documento";
      nombreAcciones = "Nome do Tipo de Documento";
      habilitado = "Habilitado";
      page = "Página";
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_tipo_comprobante" }); // Send the data to the server in JSON format.
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_tcomprobante.toLowerCase().indexOf(filtro) > -1 || 
    item.id_tcomprobante.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_tcomprobante.length > 120) {
      ...
      doc.text(datos.id_tcomprobante, 20, lineas);
      doc.text(datos.nombre_tcomprobante, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_tcomprobante.length > 100) {
...
export function tipo_comprobante_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "tipo_comprobantes");

      /* Export to file (start a download) */
      XLSX.writeFile(wb, fecha + "_Tipo_comprobante.xlsx");
    }
  };
  resultado()
}
