...
export function accion_pdf(filtro, idioma) {
  ...
  const resultado = async () => {
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_accion.length > 120) {
      ...
      doc.text(datos.id_accion, 20, lineas);
      doc.text(datos.nombre_accion, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_accion.length > 100) {
...
export function accion_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.writeFile(wb, fecha + "_Accion.xlsx");
    }
  };
  resultado()
}
