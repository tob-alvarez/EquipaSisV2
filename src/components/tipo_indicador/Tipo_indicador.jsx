...
import './table.css'
import {
  trae_tipo_indicadores,
  ayuda_tipo_indicadores,
  trae_permisos
} from "./funciones_tipo_indicador";
import { tipo_indicador_pdf, tipo_indicador_xls } from "../pdf/tipo_indicador_pdf";
...
const Tipo_indicador = () => {
  ...
  const [datos_tipo_indicador, setDatostipo_indicador] = useState([]);
  ...
  useEffect(() => {
    ...
    consultaPerfil(token)
      trae_tipo_indicadores().then((result) => setDatostipo_indicador(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_indicadores().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_indicadores().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_indicadores().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_indicadores().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tindicador.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_tindicador.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_tipo_indicador?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_indicador.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_indicador_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_indicador_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("tipo_indicador.nombre-tindicador")}</TableCell>
                <TableCell align="center">{t("tipo_indicador.estado")}</TableCell>
                <TableCell align="center">{t("tipo_indicador.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_tindicador}</TableCell>
                  <TableCell>{dato.nombre_tindicador.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_indicador.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_indicador.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
