...
import {
  ...
  ayuda_clientes,
  trae_permisos
} from "./funciones_cliente";
import { cliente_pdf, cliente_xls } from "../pdf/cliente_pdf";
...
const Cliente = () => {
  ...
  useEffect(() => {
      trae_clientes().then((result) => setDatosclientes(result));
      switch (idioma) {
        case "es":
          ayuda_clientes().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_clientes().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_clientes().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_clientes().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_cliente.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_cliente.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.base_datos.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.host.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.user.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.pass.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_clientes?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_clientes.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    cliente_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    cliente_xls(searchTerm);
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-2 container">
        ...
        <h1 className="m-0">{t("cliente.titulo")}</h1>
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("cliente.nombre-cliente")}</TableCell>
                <TableCell>{t("cliente.base_datos")}</TableCell>
            ...
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_cliente}</TableCell>
                  <TableCell>{dato.nombre_cliente.toUpperCase()}</TableCell>
                  <TableCell>{dato.base_datos.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("cliente.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("cliente.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
