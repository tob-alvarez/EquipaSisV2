...
export function persona_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    data.map((datos, index) => {
      ...
      doc.text(datos.id_persona, 20, lineas);
      doc.text(datos.nombre_persona, 34, lineas);
      doc.text(datos.nombre_tpersona, 100, lineas);
  ...
  function cabecera() {
    ...
    doc.text("Registro de Personas", 15, 12);
    ...
    doc.text("Tipo Persona", 109, 25, { align: "center" });
