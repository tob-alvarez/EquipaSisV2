...
export function tipo_componente_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    doc.setProperties({
      title: "Registro de Tipo de Componentes",
    });
    cabecera();
  ...
  //  doc.addImage(logo, "JPEG", 170, 1, 20, 15); // Agregar la imagen al PDF (X, Y, Width, Height)
    ...
    doc.text("Registro de Tipo de Componentes", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Nombre de Tipo de Componente", 80, 25, { align: "center" });
