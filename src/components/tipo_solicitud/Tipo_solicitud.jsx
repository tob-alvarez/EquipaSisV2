...
import './table.css'
import {
  trae_tipo_solicitudes,
  ayuda_tipo_solicitudes,
  trae_permisos
} from "./funciones_tipo_solicitud";
import { tipo_solicitud_pdf, tipo_solicitud_xls } from "../pdf/tipo_solicitud_pdf";
...
const Tipo_solicitud = () => {
  ...
  const [datos_tipo_solicitud, setDatostipo_solicitud] = useState([]);
  ...
  useEffect(() => {
      trae_tipo_solicitudes().then((result) => setDatostipo_solicitud(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tsolicitud.toLowerCase().includes(expresion.toLowerCase()) ||
  ...
  function buscarEnGrilla(expresiones) {
    return datos_tipo_solicitud?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_solicitud.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_solicitud_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_solicitud_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_solicitud.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_solicitud.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
