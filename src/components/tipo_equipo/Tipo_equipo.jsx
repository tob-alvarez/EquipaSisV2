...
import {
  ...
  ayuda_tipo_equipos,
  trae_permisos
} from "./funciones_tipo_equipo";
import { tipo_equipo_pdf, tipo_equipo_xls } from "../pdf/tipo_equipo_pdf";
...
const Tipo_equipo = () => {
  ...
  const [datos_tipo_equipos, setDatostipo_equipos] = useState([]);
  ...
  useEffect(() => {
    trae_tipo_equipos().then((result) => setDatostipo_equipos(result));
    switch (idioma) {
      case "es":
        ayuda_tipo_equipos().then((ayuda) => setAyuda(ayuda[0].texto));
        break;
      case "en":
        ayuda_tipo_equipos().then((ayuda) => setAyuda(ayuda[0].texto_en));
        break;
      case "por":
        ayuda_tipo_equipos().then((ayuda) => setAyuda(ayuda[0].texto_por));
        break;
      default:
        ayuda_tipo_equipos().then((ayuda) => setAyuda(ayuda[0].texto));
    }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tequipo.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_tequipo.toLowerCase().includes(expresion.toLowerCase()) ||
  ...
  function buscarEnGrilla(expresiones) {
    return datos_tipo_equipos?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_equipos.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_equipo_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_equipo_xls(searchTerm);
  };
        ...
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            ...
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{ textAlign: 'center' }}>{dato.id_tequipo}</TableCell>
                  <TableCell >{dato.nombre_tequipo.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            <Typography variant="p" className="col-3 align-self-center">
              {t("tipo_equipo.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
              {t("tipo_equipo.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
