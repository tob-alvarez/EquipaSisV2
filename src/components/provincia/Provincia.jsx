...
import {
  ...
  ayuda_provincias,
  trae_permisos
} from "./funciones_provincia";
import { provincia_pdf, provincia_xls } from "../pdf/provincia_pdf";
...
const Provincia = () => {
  ...
  const [datos_provincias, setDatosprovincias] = useState([]);
  ...
  useEffect(() => {
    ...
    consultaPerfil(token)
      trae_provincias().then((result) => setDatosprovincias(result));
      switch (idioma) {
        case "es":
          ayuda_provincias().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_provincias().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_provincias().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_provincias().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_provincia.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_provincia.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_provincias?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_provincias.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    provincia_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    provincia_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("provincia.nombre-provincia")}</TableCell>
                <TableCell align="center">{t("provincia.estado")}</TableCell>
                <TableCell align="center">{t("provincia.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_provincia}</TableCell>
                  <TableCell>{dato.nombre_provincia.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("provincia.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("provincia.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
