...
import {
  ...
  ayuda_marcas,
  trae_permisos
} from "./funciones_marca";
import { marca_pdf, marca_xls } from "../pdf/marca_pdf";
...
const Marca = () => {
  ...
  const [datos_marcas, setDatosmarcas] = useState([]);
  ...
  useEffect(() => {
    ...
    consultaPerfil(token)
      trae_marcas().then((result) => setDatosmarcas(result));
      switch (idioma) {
        case "es":
          ayuda_marcas().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_marcas().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_marcas().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_marcas().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_marca.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_marca.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_marcas?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_marcas.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    marca_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    marca_xls(searchTerm);
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-2 container">
        ...
        <h1 className="m-0">{t("marca.titulo")}</h1>
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("marca.nombre-marca")}</TableCell>
                <TableCell align="center">{t("marca.estado")}</TableCell>
                <TableCell align="center">{t("marca.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_marca}</TableCell>
                  <TableCell>{dato.nombre_marca.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("marca.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("marca.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
