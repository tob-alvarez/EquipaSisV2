...
export function provincia_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    ...
    if (idioma === 'es') {
      ...
      nombreAcciones = "Nombre de Provincia";
      habilitado = "Habilitada";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Province";
      nombreAcciones = "Province Name";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas da Províncias";
      nombreAcciones = "Nome da Província";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Relatório em";
    } else {
      titulo = "Registro de Provincias";
      nombreAcciones = "Nombre de Provincia";
      habilitado = "Habilitada";
      page = "Página";
      reporte = "Reporte al"
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_provincia" }); // Send the data to the server in JSON format.
    const endpoint = "https://v2.equipasis.com/api/provincia.php"; // API endpoint where we send form data.
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_provincia.toLowerCase().indexOf(filtro) > -1 || 
    item.id_provincia.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_provincia.length > 120) {
      ...
      doc.text(datos.id_provincia, 20, lineas);
      doc.text(datos.nombre_provincia, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_provincia.length > 100) {
...
export function provincia_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.writeFile(wb, fecha + "_Provincia.xlsx");
    }
  };
  resultado()
}
