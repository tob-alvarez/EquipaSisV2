...
export function tipo_adjunto_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    let reporte;
    if (idioma === 'es') {
      titulo = "Registro de Tipo de Adjuntos";
      nombreAcciones = "Nombre de Tipo de Adjunto";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Attachment Type";
      nombreAcciones = "Attachment Type";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas de Tipo de Anexo";
      nombreAcciones = "Nome da Tipo de Anexo";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Relatório em";
    } else {
      titulo = "Registro de Tipo de Adjuntos";
      nombreAcciones = "Nombre de Tipo de Adjuntos";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Reporte al"
    }
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_tipo_adjunto" }); // Send the data to the server in JSON format.
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_tadjunto.toLowerCase().indexOf(filtro) > -1 || 
    item.id_tadjunto.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_tadjunto.length > 120) {
      ...
      doc.text(datos.id_tadjunto, 20, lineas);
      doc.text(datos.nombre_tadjunto, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_tadjunto.length > 100) {
...
export function tipo_adjunto_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "tipo_adjuntos");

      /* Export to file (start a download) */
      XLSX.writeFile(wb, fecha + "_Tipo_Adjunto.xlsx");
    }
  };
  resultado()
}
