...
import {
  ...
  ayuda_tipo_personas,
  trae_permisos
} from "./funciones_tipo_persona";
import { tipo_persona_pdf, tipo_persona_xls } from "../pdf/tipo_persona_pdf";
...
const Tipo_persona = () => {
  ...
  const [datos_tipo_persona, setDatostipo_persona] = useState([]);
  ...
  useEffect(() => {
      trae_tipo_personas().then((result) => setDatostipo_persona(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_personas().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_personas().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_personas().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_personas().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tpersona.toLowerCase().includes(expresion.toLowerCase()) ||
  ...
  function buscarEnGrilla(expresiones) {
    return datos_tipo_persona?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_persona.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_persona_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_persona_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_persona.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_persona.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
