...
export function organizacion_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    const endpoint = "https://v2.equipasis.com/api_desarrollo/organizacion.php"; // API endpoint where we send form data.
    ...
    data.map((datos, index) => {
      ...
      doc.text(datos.corto_organizacion, 100, lineas);
  ...
  //  doc.addImage(logo, "JPEG", 170, 1, 20, 15); // Agregar la imagen al PDF (X, Y, Width, Height)
    ...
    doc.text("Registro de Organizaciones", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Nombre del Organización", 50, 25, { align: "center" });
