...
import {
  ...
  ayuda_opciones,
  trae_permisos
} from "./funciones_opcion";
import { opcion_pdf, opcion_xls } from "../pdf/opcion_pdf";
...
const Opcion = () => {
  ...
  const [datos_opciones, setDatosopciones] = useState([]);
  ...
  useEffect(() => {
      trae_opciones().then((result) => setDatosopciones(result));
      switch (idioma) {
        case "es":
          ayuda_opciones().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_opciones().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_opciones().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_opciones().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarEnGrilla(expresiones) {
    return datos_opciones?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_opciones.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    opcion_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    opcion_xls(searchTerm);
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-2 container">
        ...
        <h1 className="m-0">{t("opcion.titulo")}</h1>
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("opcion.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("opcion.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
