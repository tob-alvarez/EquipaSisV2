...
import {
  ...
  ayuda_tipo_adjuntos,
  trae_permisos
} from "./funciones_tipo_adjunto";
import { tipo_adjunto_pdf, tipo_adjunto_xls } from "../pdf/tipo_adjunto_pdf";
...
const Tipo_adjunto = () => {
  ...
  const [datos_tipo_adjunto, setDatostipo_adjunto] = useState([]);
  ...
  useEffect(() => {
      trae_tipo_adjuntos().then((result) => setDatostipo_adjunto(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_adjuntos().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_adjuntos().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_adjuntos().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_adjuntos().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tadjunto.toLowerCase().includes(expresion.toLowerCase()) ||
  ...
  function buscarEnGrilla(expresiones) {
    return datos_tipo_adjunto?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_adjunto.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_adjunto_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_adjunto_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("tipo_adjunto.nombre-tadjunto")}</TableCell>
                <TableCell align="center">{t("tipo_adjunto.estado")}</TableCell>
                <TableCell align="center">{t("tipo_adjunto.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_tadjunto}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_adjunto.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_adjunto.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
