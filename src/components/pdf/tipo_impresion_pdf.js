...
export function tipo_impresion_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    let reporte;
    if (idioma === 'es') {
      titulo = "Registro de Tipo de Impresiones";
      nombreAcciones = "Nombre de Tipo de Impresión";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Print Type";
      nombreAcciones = "Print Type";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas do Tipo de Impressões";
      nombreAcciones = "Nome do Tipo de Impressão";
      habilitado = "Habilitado";
      page = "Página";
  ...
  const resultado = async () => {
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_timpresion.toLowerCase().indexOf(filtro) > -1 || 
    item.id_timpresion.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_timpresion.length > 120) {
      ...
      doc.text(datos.id_timpresion, 20, lineas);
      doc.text(datos.nombre_timpresion, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_timpresion.length > 100) {
...
export function tipo_impresion_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "tipo_impresiones");

      /* Export to file (start a download) */
      XLSX.writeFile(wb, fecha + "_Tipo_impresion.xlsx");
    }
  };
  resultado()
}
