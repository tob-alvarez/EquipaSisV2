...
export function tipo_control_pdf(filtro, idioma) {
  let titulo;
  let nombreAcciones;
  let unidad;
  let habilitado;
  ...
  let reporte;
  if (idioma === 'es') {
    titulo = "Registro de Tipo de Controles";
    nombreAcciones = "Nombre de Tipo de Control";
    unidad = "Unidad";
    habilitado = "Habilitada";
    page = "Página";
    reporte = "Reporte al"
  } else if (idioma === 'en') {
    titulo = "Records of Control Type";
    nombreAcciones = "Control Type Name";
    unidad = "Unit";
    habilitado = "Enabled";
    page = "Page";
    reporte = "Report as of";
  } else if (idioma === 'por') {
    titulo = "Datas de Tipo de Controle";
    nombreAcciones = "Nome do Tipo de Controle";
    unidad = "Unidade";
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_tipo_control" }); // Send the data to the server in JSON format.
    ...
    data = result.datos;
    data = data.filter(item => item.id_tcontrol.toLowerCase().indexOf(filtro) > -1 ||
    item.nombre_tcontrol.toLowerCase().indexOf(filtro) > -1 ||
    item.unidad.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_tcontrol.length > 120) {
      ...
      doc.text(datos.id_tcontrol, 20, lineas);
      doc.text(datos.nombre_tcontrol, 34, lineas);
      doc.text(datos.unidad, 100, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_tcontrol.length > 100) {
...
export function tipo_control_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_tcontrol.toLowerCase().indexOf(filtro) > -1 ||
    item.id_tcontrol.toLowerCase().indexOf(filtro) > -1 ||
    item.unidad.toLowerCase().indexOf(filtro) > -1 ||
    item.habilita.toLowerCase().indexOf(filtro) > -1);
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "tipo_control");

      /* Export to file (start a download) */
      XLSX.writeFile(wb, fecha + "_Tipo_control.xlsx");
    }
  };
  resultado()
}
