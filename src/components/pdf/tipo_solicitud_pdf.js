...
export function tipo_solicitud_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    doc.setProperties({
      title: "Registro de Tipo de Solicitudes",
    });
    cabecera();
  ...
  function cabecera() {
    ...
    doc.text("Registro de Tipo de Solicitudes", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Nombre de Tipo de Solicitud", 80, 25, { align: "center" });
