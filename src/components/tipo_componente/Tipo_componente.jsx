...
import './table.css'
import {
  trae_tipo_componentes,
  ayuda_tipo_componentes,
  trae_permisos
} from "./funciones_tipo_componente";
import { tipo_componente_pdf, tipo_componente_xls } from "../pdf/tipo_componente_pdf";
...
const Tipo_componente = () => {
  const [t] = useTranslation("global")
  const [datos_tipo_componente, setDatostipo_componente] = useState([]);
  ...
  useEffect(() => {
    ...
    consultaPerfil(token)
      trae_tipo_componentes().then((result) => setDatostipo_componente(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_componentes().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_componentes().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_componentes().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_componentes().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tcomponente.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_tcomponente.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_tipo_componente?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_componente.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_componente_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_componente_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("tipo_componente.nombre-tcomponente")}</TableCell>
                <TableCell align="center">{t("tipo_componente.estado")}</TableCell>
                <TableCell align="center">{t("tipo_componente.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_tcomponente}</TableCell>
                  <TableCell>{dato.nombre_tcomponente.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_componente.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_componente.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
...
export default Tipo_componente