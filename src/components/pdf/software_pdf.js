...
export function software_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    const endpoint = "https://v2.equipasis.com/api_desarrollo/software.php"; // API endpoint where we send form data.
    ...
    data.map((datos, index) => {
      ...
      doc.text(datos.nombre_software, 34, lineas);
      doc.text(datos.corto_software, 100, lineas);
  ...
  //  doc.addImage(logo, "JPEG", 170, 1, 20, 15); // Agregar la imagen al PDF (X, Y, Width, Height)
    ...
    doc.text("Registro de Softwares", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Nombre del Software", 50, 25, { align: "center" });
