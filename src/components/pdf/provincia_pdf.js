...
export function provincia_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    const endpoint = "https://v2.equipasis.com/api_desarrollo/provincia.php"; // API endpoint where we send form data.
  ...
  //  doc.addImage(logo, "JPEG", 170, 1, 20, 15); // Agregar la imagen al PDF (X, Y, Width, Height)
    ...
    doc.text("Registro de Provincias", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Nombre de Provincia", 80, 25, { align: "center" });
