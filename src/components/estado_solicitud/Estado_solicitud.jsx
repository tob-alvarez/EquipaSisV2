...
import './table.css'
import {
  trae_estado_solicitudes,
  ayuda_estado_solicitudes,
  trae_permisos
} from "./funciones_estado_solicitud";
import { estado_solicitud_pdf, estado_solicitud_xls } from "../pdf/estado_solicitud_pdf";
...
const Estado_solicitud = () => {
  ...
  const [datos_estado_solicitudes, setDatosestado_solicitudes] = useState([]);
  ...
  useEffect(() => {
    ...
    consultaPerfil(token)
      trae_estado_solicitudes().then((result) => setDatosestado_solicitudes(result));
      switch (idioma) {
        case "es":
          ayuda_estado_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_estado_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_estado_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_estado_solicitudes().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_estado_solicitud.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_estado_solicitud.toLowerCase().includes(expresion.toLowerCase()) ||
  ...
  function buscarEnGrilla(expresiones) {
    return datos_estado_solicitudes?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_estado_solicitudes.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    estado_solicitud_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    estado_solicitud_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("estado_solicitud.nombre-estado_solicitud")}</TableCell>
                <TableCell align="center">{t("estado_solicitud.color")}</TableCell>
                <TableCell align="center">{t("estado_solicitud.estado")}</TableCell>
                <TableCell align="center">{t("estado_solicitud.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_estado_solicitud}</TableCell>
                  <TableCell >{dato.nombre_estado_solicitud.toUpperCase()}</TableCell>
                  <TableCell sx={{
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("estado_solicitud.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("estado_solicitud.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
