...
export function tipo_control_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  //  doc.addImage(logo, "JPEG", 170, 1, 20, 15); // Agregar la imagen al PDF (X, Y, Width, Height)
    ...
    doc.text("Registro de Tipos de Control", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Tipo de Control", 50, 25, { align: "center" });
...
export function tipo_control_xls(filtro) {
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "tipo_controles");
