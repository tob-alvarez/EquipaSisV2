...
export function prioridad_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    let reporte;
    if (idioma === 'es') {
      titulo = "Registro de Prioridades";
      nombreAcciones = "Nombre de Prioridad";
      habilitado = "Habilitada";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Priorities";
      nombreAcciones = "Priority Name";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas de Prioridades";
      nombreAcciones = "Nome de Prioridad";
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_prioridad" }); // Send the data to the server in JSON format.
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_prioridad.toLowerCase().indexOf(filtro) > -1 || 
    item.id_prioridad.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_prioridad.length > 120) {
      ...
      doc.text(datos.id_prioridad, 20, lineas);
      doc.text(datos.nombre_prioridad, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_prioridad.length > 100) {
...
export function prioridad_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.writeFile(wb, fecha + "_Prioridad.xlsx");
    }
  };
  resultado()
}
