...
import { pais_pdf, pais_xls } from "../pdf/pais_pdf";
...
const Pais = () => {
  ...
  const [datos_paises, setDatospaises] = useState([]);
  ...
  useEffect(() => {
      ...
      switch (idioma) {
        case "es":
          ...
          break;
        case "en":
          ayuda_paises().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_paises().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_paises().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_pais.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_pais.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_paises?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_paises.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-2 container">
        ...
        <h1 className="m-0">{t("pais.titulo")}</h1>
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("pais.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("pais.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
