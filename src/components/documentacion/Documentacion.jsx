...
import './table.css'
import {
  trae_documentaciones,
  ayuda_documentaciones,
  trae_permisos
} from "./funciones_documentacion";
import { documentacion_pdf, documentacion_xls } from "../pdf/documentacion_pdf";
...
const Documentacion = () => {
  ...
  const [datos_documentaciones, setDatosdocumentaciones] = useState([]);
  ...
  useEffect(() => {
      trae_documentaciones().then((result) => setDatosdocumentaciones(result));
      switch (idioma) {
        case "es":
          ayuda_documentaciones().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_documentaciones().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_documentaciones().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_documentaciones().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarEnGrilla(expresiones) {
    return datos_documentaciones?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_documentaciones.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    documentacion_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    documentacion_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("documentacion.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("documentacion.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
