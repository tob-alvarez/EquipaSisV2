...
import {
  trae_estado_equipos,
  ayuda_estado_equipos,
  trae_permisos
} from "./funciones_estado_equipo";
import { estado_equipo_pdf, estado_equipo_xls } from "../pdf/estado_equipo_pdf";
...
const Estado_equipo = () => {
  ...
  const [datos_estado_equipos, setDatosestado_equipos] = useState([]);
  ...
  useEffect(() => {
      ...
      trae_estado_equipos().then((result) => setDatosestado_equipos(result));
      switch (idioma) {
        case "es":
          ayuda_estado_equipos().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_estado_equipos().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_estado_equipos().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_estado_equipos().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_estado.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_estado.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.corto_estado.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.color.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_estado_equipos?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_estado_equipos.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    estado_equipo_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    estado_equipo_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("estado_equipo.nombre-estado")}</TableCell>
                <TableCell>{t("estado_equipo.corto_estado")}</TableCell>
                <TableCell align="center">{t("estado_equipo.color")}</TableCell>
                <TableCell align="center">{t("estado_equipo.estado")}</TableCell>
                <TableCell align="center">{t("estado_equipo.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_estado}</TableCell>
                  <TableCell >{dato.nombre_estado.toUpperCase()}</TableCell>
                  <TableCell >{dato.corto_estado.toUpperCase()}</TableCell>
                  <TableCell sx={{
                    backgroundColor: `${dato.color}`,
                    textAlign: "center"
                  }}>{dato.color.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("estado_equipo.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("estado_equipo.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
