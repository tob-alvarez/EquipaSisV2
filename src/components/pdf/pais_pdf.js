...
export function pais_pdf(filtro) {
  ...
  const resultado = async () => {
    ...
    const endpoint = "https://v2.equipasis.com/api_desarrollo/pais.php"; // API endpoint where we send form data.
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_pais.length > 120) {
      ...
      doc.text(datos.id_pais, 20, lineas);
      doc.text(datos.nombre_pais, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_pais.length > 100) {
...
export function pais_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.writeFile(wb, fecha + "_Pais.xlsx");
    }
  };
  resultado()
}
