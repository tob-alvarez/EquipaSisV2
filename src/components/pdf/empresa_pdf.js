...
export function empresa_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    ...
    if (idioma === 'es') {
      ...
      nombreAcciones = "Nombre de Empresa";
      habilitado = "Habilitada";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Companies";
      nombreAcciones = "Company Name";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas de Empresas";
      nombreAcciones = "Nome da Empresa";
  ...
  const resultado = async () => {
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_empresa.toLowerCase().indexOf(filtro) > -1 || 
    item.id_empresa.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_empresa.length > 120) {
      ...
      doc.text(datos.id_empresa, 20, lineas);
      doc.text(datos.nombre_empresa, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_empresa.length > 100) {
...
export function empresa_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.writeFile(wb, fecha + "_Empresa.xlsx");
    }
  };
  resultado()
}
