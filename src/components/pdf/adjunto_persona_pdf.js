...
export function adjunto_persona_pdf(filtro) {
  const doc = new jsPDF({
    orientation: "p",
    unit: "mm",
    format: "a4",
  ...
  const resultado = async () => {
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_adjunto_persona.toLowerCase().indexOf(filtro) > -1);
    doc.setProperties({
      title: "Registro de Adjuntos de Personas",
    });
    cabecera();
    data.map((datos, index) => {
      ...
      doc.text(datos.id_adjunto_persona, 20, lineas);
      doc.text(datos.nombre_persona, 34, lineas);
      doc.text(datos.nombre_archivo, 100, lineas);
  ...
  function cabecera() {
    ...
    doc.text("Registro de Adjuntos de Personas", 15, 12);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
    doc.text("ID", 22, 25, { align: "center" });
    doc.line(30, 19.8, 30, 27.2);
    doc.text("Nombre de la Persona", 50, 25, { align: "center" });
    doc.line(95, 19.8, 95, 27.2);
    doc.text("Nombre Archivo", 109, 25, { align: "center" });
...
export function adjunto_persona_xls(filtro) {
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "adjunto_personas");
