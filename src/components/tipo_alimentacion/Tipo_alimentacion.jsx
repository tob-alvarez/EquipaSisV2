...
import './table.css'
import {
  trae_tipo_alimentaciones,
  ayuda_tipo_alimentaciones,
  trae_permisos
} from "./funciones_tipo_alimentacion";
import { tipo_alimentacion_pdf, tipo_alimentacion_xls } from "../pdf/tipo_alimentacion_pdf";
...
const Tipo_alimentacion = () => {
  const [t] = useTranslation("global")
  const [datos_tipo_alimentacion, setDatostipo_alimentacion] = useState([]);
  ...
  useEffect(() => {
      trae_tipo_alimentaciones().then((result) => setDatostipo_alimentacion(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_alimentaciones().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_alimentaciones().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_alimentaciones().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_alimentaciones().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_talimentacion.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_talimentacion.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_tipo_alimentacion?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_alimentacion.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_alimentacion_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_alimentacion_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("tipo_alimentacion.nombre-talimentacion")}</TableCell>
                <TableCell align="center">{t("tipo_alimentacion.estado")}</TableCell>
                <TableCell align="center">{t("tipo_alimentacion.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_talimentacion}</TableCell>
                  <TableCell>{dato.nombre_talimentacion.toUpperCase()}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_alimentacion.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_alimentacion.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
...
export default Tipo_alimentacion