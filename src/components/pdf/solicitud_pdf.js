...
export function solicitud_pdf(filtro, idioma) {
    let titulo;
    let detallesolicita;
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_solicitud" }); // Send the data to the server in JSON format.
  ...
  //  doc.text(fecha_hora , 280, 25, { align: "center" });
  //  doc.line(282, 19.8, 282, 27.2);
...
export function solicitud_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.writeFile(wb, fecha + "_Solicitud.xlsx");
    }
  };
  resultado()
}
