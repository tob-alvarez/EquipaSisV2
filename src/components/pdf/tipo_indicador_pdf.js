...
export function tipo_indicador_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    let reporte;
    if (idioma === 'es') {
      titulo = "Registro de Tipo de Indicadores";
      nombreAcciones = "Nombre de Tipo de Indicador";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Indicator Type";
      nombreAcciones = "Indicator Type";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas do Tipo de Indicador";
      nombreAcciones = "Nome do Tipo de Indicador";
      habilitado = "Habilitado";
      page = "Página";
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_tipo_indicador" }); // Send the data to the server in JSON format.
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_tindicador.toLowerCase().indexOf(filtro) > -1 || 
    item.id_tindicador.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_tindicador.length > 120) {
      ...
      doc.text(datos.id_tindicador, 20, lineas);
      doc.text(datos.nombre_tindicador, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_tindicador.length > 100) {
...
export function tipo_indicador_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "tipo_indicadores");

      /* Export to file (start a download) */
      XLSX.writeFile(wb, fecha + "_Tipo_indicador.xlsx");
    }
  };
  resultado()
}
