...
import {
  ...
  ayuda_tipo_tareas,
  trae_permisos
} from "./funciones_tipo_tarea";
import { tipo_tarea_pdf, tipo_tarea_xls } from "../pdf/tipo_tarea_pdf";
...
const Tipo_tarea = () => {
  ...
  const [datos_tipo_tarea, setDatostipo_tarea] = useState([]);
  ...
  useEffect(() => {
      trae_tipo_tareas().then((result) => setDatostipo_tarea(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_tareas().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_tareas().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_tareas().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_tareas().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_ttarea.toLowerCase().includes(expresion.toLowerCase()) ||
  ...
  function buscarEnGrilla(expresiones) {
    return datos_tipo_tarea?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_tarea.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_tarea_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_tarea_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("tipo_tarea.nombre-ttarea")}</TableCell>
                <TableCell align="center">{t("tipo_tarea.estado")}</TableCell>
                <TableCell align="center">{t("tipo_tarea.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {currentItems.map((dato, index) => (
                ...
                >
                  <TableCell sx={{textAlign: 'center'}}>{dato.id_ttarea}</TableCell>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_tarea.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_tarea.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
