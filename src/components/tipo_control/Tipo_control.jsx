...
import {
  trae_tipo_controles,
  ayuda_tipo_controles,
  trae_permisos
} from "./funciones_tipo_control";
import { tipo_control_pdf, tipo_control_xls } from "../pdf/tipo_control_pdf";
...
const Tipo_control = () => {
  ...
  const [datos_tipo_control, setDatostipo_control] = useState([]);
  ...
  useEffect(() => {
      trae_tipo_controles().then((result) => setDatostipo_control(result));
      switch (idioma) {
        case "es":
          ayuda_tipo_controles().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_tipo_controles().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_tipo_controles().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_tipo_controles().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_tcontrol.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.nombre_tcontrol.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.unidad.toLowerCase().includes(expresion.toLowerCase()) ||
        grilla.habilita_3.toLowerCase().includes(expresion.toLowerCase())
    );
  }

  function buscarEnGrilla(expresiones) {
    return datos_tipo_control?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_tipo_control.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    tipo_control_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    tipo_control_xls(searchTerm);
  };
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          <Table aria-label="material ui table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t("tipo_control.nombre-tcontrol")}</TableCell>
                <TableCell>{t("tipo_control.unidad")}</TableCell>                
                <TableCell align="center">{t("tipo_control.estado")}</TableCell>
                <TableCell align="center">{t("tipo_control.acciones")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("tipo_control.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("tipo_control.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
