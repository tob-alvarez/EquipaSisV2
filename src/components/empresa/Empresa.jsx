...
import { empresa_pdf, empresa_xls } from "../pdf/empresa_pdf";
...
const Empresa = () => {
  ...
  const [datos_empresas, setDatosempresas] = useState([]);
  ...
  useEffect(() => {
      trae_empresas().then((result) => setDatosempresas(result));
      switch (idioma) {
        case "es":
          ayuda_empresas().then((ayuda) => setAyuda(ayuda[0].texto));
          break;
        case "en":
          ayuda_empresas().then((ayuda) => setAyuda(ayuda[0].texto_en));
          break;
        case "por":
          ayuda_empresas().then((ayuda) => setAyuda(ayuda[0].texto_por));
          break;
        default:
          ayuda_empresas().then((ayuda) => setAyuda(ayuda[0].texto));
      }
  ...
  function buscarPorExpresiones(grilla, expresiones) {
    ...
    return expresionesArray.some(
      (expresion) =>
        grilla.id_empresa.toLowerCase().includes(expresion.toLowerCase()) ||
  ...
  function buscarEnGrilla(expresiones) {
    return datos_empresas?.filter((grilla) => buscarPorExpresiones(grilla, expresiones));
  }
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= Math.ceil(datos_empresas.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  const printInfoProcess = () => {
    let idioma = localStorage.getItem("language")
    console.log(idioma)
    empresa_pdf(searchTerm, idioma);
  };
  const downloadInfo = () => {
    empresa_xls(searchTerm);
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-2 container">
        ...
        <h1 className="m-0">{t("empresa.titulo")}</h1>
      ...
      <div className="container mt-3 mb-5">
        <TableContainer component={Paper}>
          ...
          <div className="pagination justify-content-center mt-2 align-items-center p-2">
            ...
            {t("empresa.pagina")} {currentPage} {t("accion.de")} {Math.ceil(filteredItems.length / itemsPerPage)}
            </Typography>
            <Typography variant="p" className="align-self-center">
            {t("empresa.registros")} {filteredItems.length}
            </Typography>
          </div>
        </TableContainer>
      </div>
