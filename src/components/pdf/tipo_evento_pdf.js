...
export function tipo_evento_pdf(filtro, idioma) {
    let titulo;
    let nombreAcciones;
    let habilitado;
    let page; 
    let reporte;
    if (idioma === 'es') {
      titulo = "Registro de Tipo de Eventos";
      nombreAcciones = "Nombre de Tipo de Evento";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Reporte al"
    } else if (idioma === 'en') {
      titulo = "Records of Event Type";
      nombreAcciones = "Event Type Name";
      habilitado = "Enabled";
      page = "Page";
      reporte = "Report as of";
    } else if (idioma === 'por') {
      titulo = "Datas do Tipo de Evento";
      nombreAcciones = "Nome do Tipo de Evento";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Relatório em";
    } else {
      titulo = "Registro de Tipo de Evento";
      nombreAcciones = "Nombre de Tipo de Evento";
      habilitado = "Habilitado";
      page = "Página";
      reporte = "Reporte al"
  ...
  const resultado = async () => {
    const JSONdata = JSON.stringify({ tarea: "imprime_tipo_evento" }); // Send the data to the server in JSON format.
    ...
    data = result.datos;
    data = data.filter(item => item.nombre_tevento.toLowerCase().indexOf(filtro) > -1 || 
    item.id_tevento.toLowerCase().indexOf(filtro) > -1 ||
    ...
    data.map((datos, index) => {
      if (index % 2 == 0 && datos.nombre_tevento.length > 120) {
      ...
      doc.text(datos.id_tevento, 20, lineas);
      doc.text(datos.nombre_tevento, 34, lineas);

      if (datos.habilita == 0) habilita = "NO";
      else habilita = "SI";
      doc.text(habilita, 168, lineas);

      if (datos.nombre_tevento.length > 100) {
...
export function tipo_evento_xls(filtro) {
  let data = [];
  ...
  const resultado = async () => {
    ...
    if (data.length != 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "tipo_eventos");

      /* Export to file (start a download) */
      XLSX.writeFile(wb, fecha + "_Tipo_evento.xlsx");
    }
  };
  resultado()
}
